...
namespace pizda {
	...
	uint8_t PacketParser::readValueCountAndValidateChecksum(BitStream& bitStream, uint8_t valueCountBitCount, uint8_t valueBitCount) {
		...
		ESP_LOGI(_logTag, "value count: %d", valueCount);
	...
	uint8_t PacketParser::parseOne(uint8_t* buffer) {
		...
		ESP_LOGI(_logTag, "packet type: %d", static_cast<uint8_t>(packetType));

		// Payload parsing
		if (!onParse(bitStream, packetType)) {
			ESP_LOGI(_logTag, "parsing failed");
		...
		ESP_LOGI(_logTag, "payload length: %d, total length: %d", payloadLength, packetLength);
		ESP_LOGI(_logTag, "-------- End --------");
